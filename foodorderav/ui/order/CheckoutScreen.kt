...
fun CheckoutScreen(
    ...
    profileViewModel: ProfileViewModel,  // ← THÊM PROFILEVIEWMODEL
    onNavigateBack:  () -> Unit,
    onOrderSuccess: () -> Unit
) {
    ...
    // ========== LOCAL STATE CHO FORM (có thể chỉnh sửa mà không ảnh hưởng data gốc) ==========
    // Khởi tạo 1 LẦN DUY NHẤT từ profile, sau đó user tự do chỉnh sửa
    ...
    // Load user profile khi vào màn hình
    LaunchedEffect(Unit) {
    ...
    Scaffold(
        ...
        Column(
            ...
            Row(
                ...
                // ========== HIỂN THỊ THÔNG BÁO NẾU ĐÃ TỰ ĐỘNG ĐIỀN ==========
                if (! currentUser?.fullName.isNullOrBlank()) {
                    Text(
                        text = "✓ Đã điền từ hồ sơ",
            ...
            OutlinedTextField(
                ...
                modifier = Modifier.fillMaxWidth(),
                singleLine = true,
                // Hiển thị supportingText nếu khác với data gốc
                supportingText = if (customerName != (currentUser?.fullName ?: "") &&
                    !currentUser?.fullName.isNullOrBlank()) {
                    { Text("Đã chỉnh sửa (không ảnh hưởng hồ sơ)") }
                } else null
            )
            ...
            OutlinedTextField(
                ...
                singleLine = true,
                supportingText = if (phone != (currentUser?.phone ?: "") &&
            ...
            OutlinedTextField(
                ...
                minLines = 2,
                maxLines = 3,
                supportingText = if (address != (currentUser?.address ?: "") &&
                    !currentUser?.address.isNullOrBlank()) {
                    { Text("Đã chỉnh sửa (không ảnh hưởng hồ sơ)") }
                } else null
            )
